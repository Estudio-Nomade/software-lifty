...
-- never be NULL. Backfill any legacy rows, then enforce NOT NULL. collectTrip
-- already rejects non-positive fares, so backfilled 0 values are safe and will
-- surface loudly if ever collected.
