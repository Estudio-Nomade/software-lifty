...
describe('connectBlockedFeedback', () => {
  it('maps local blockers to polished Spanish copy', () => {
    ...
    expect(feedbackForConnectBlock('stickers_overdue').title).toBe('Cuenta suspendida');
    expect(feedbackForConnectBlock('no_location').title).toBe('Falta tu ubicación');
  });

  it('maps STICKERS_PICKUP_OVERDUE API errors', () => {
    const err = new ApiError({
      error: {
        code: 'STICKERS_PICKUP_OVERDUE',
        message:
    ...
    expect(feedback.title).toBe('Cuenta suspendida');
    expect(feedback.message).toMatch(/30 días/i);
